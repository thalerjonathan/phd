...
sgDt a dt = PA.updateState aAfterAction (\s -> s { sgSumPayoff = 0.0 } )
    where
        po = sgSumPayoff (PA.state a)
        newState = sgMaxPayoffState (PA.state a)
        oldState = sgCurrState (PA.state a)
        aAfterPayoff = PA.updateState a (\s -> s { sgCurrState = newState, sgPrevState = oldState } )
        aAfterBroad = PA.broadcastMsgToNeighbours aAfterPayoff (NeighbourPayoff newState po)
        aAfterAction = PA.broadcastMsgToNeighbours aAfterBroad (NeighbourAction newState)
