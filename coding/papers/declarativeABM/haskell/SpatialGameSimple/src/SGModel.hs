...
localPayoff cs c = c { sgLocalPayoff = lp }
    where
        ns = cellNeighbours c cs
        lp = foldl (\payoffSum n -> payoffSum  + (payoffWith c n)) 0.0 ns

bestWin :: CellContainer -> SGCell -> SGCell
bestWin cs c = c { sgCurrState = currState, sgPrevState = prevState }
    where
        ns = cellNeighbours c cs
        bestCell = foldl (\best n -> compareWith best n) c ns
...
cellNeighbours :: SGCell -> CellContainer -> [SGCell]
cellNeighbours c cs = map (\i -> fromJust $ Map.lookup i cs) nids
    where
