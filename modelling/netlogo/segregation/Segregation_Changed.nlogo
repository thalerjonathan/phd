...
to setup
  ...
  calculate-empty-patches
  update-turtles
  update-globals
  reset-ticks
end

to calculate-empty-patches
  let x (count (patches with [ count turtles-here = 0 ]))
  show x
end
...
to move-unhappy-turtles
  ask turtles with [ not happy? ]
    [ if random-float 1 >= (%-accept-situation / 100)
...
to find-new-spot-local
  rt random-float 360
  fd random-float 10
  if any? other turtles-here [ find-new-spot-local ] ;; keep going until we find an unoccupied patch
  move-to patch-here  ;; move to center of patch
end

;; moves to first free spot in neighbourhood, if no free spot, then stays
to find-new-spot-neighbour
  let ps (patch-set neighbors)
  move-to (find-first-free-patch ps)
end

to find-new-spot-global [originalPatch retries]
  let randomPatch one-of patches

  move-to randomPatch

  let local-similar-nearby count (turtles-on neighbors) with [ color = [ color ] of myself ]
  if not ((local-similar-nearby > similar-nearby) and (not any? other turtles-here))
      [ ifelse retries > 0
          [find-new-spot-global originalPatch (retries - 1)]
          [ move-to originalPatch ]
      ]
end

to-report find-first-free-patch [ps]
  let p patch-here
