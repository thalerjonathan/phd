...
\begin{frame}{How-To implement ABS?}
  \begin{block}{Established, state-of-the-art approach in ABS}
	Object-Oriented Programming in Python (on the rise!), Java,...
  \end{block}
  
  \begin{block}{What we want is (pure) functional programming}
	\begin{itemize}
	  \item Reproducibility guaranteed at compile time
	  \item Declarative, reasoning, parallelism, concurrency, property-based testing,...
	\end{itemize}
  \end{block}
...
\begin{frame}{Functional Reactive Programming (FRP)}
  \begin{itemize}
    \item Implement continuous- \& discrete-time systems in functional programming
 	\item Central concept: Signal Function (SF), process over time, maps signal to signal
...
\begin{frame}{Reflection}
  \begin{itemize}
	...
	\item \textbf{Stochasticity} - inherently stochastic simulation.
...
\begin{frame}{Where is the benefit?}
  \begin{block}{Same as System Dynamics}
    Where is the Environment?
  \end{block} 
  
  \begin{block}{Adding an Environment}
    \begin{itemize}
      \item Running SFs happens in \textit{'parallel'} => multiple copies of environment
  ...
  \begin{block}{Solution}
    Use Monadic Stream Functions (MSFs) \\
  	Conceptually, Signal Functions with monadic context
  \end{block}
\end{frame}
...
\begin{frame}{Future Work}
  \begin{block}{Property-based testing for verification in ABS}
    \begin{itemize}
      \item Completely untouched (property-based testing exists in Java, Python)
      \item Might offer huge benefits e.g. formalising hypotheses
    \end{itemize}
  \end{block}
  
  \begin{block}{Dependent Types in ABS (using Idris)}
  	\begin{itemize}
  	  %\item No research exists so far (to our best knowledge)
  	  \item Safe environment access
	  \item Safe state-transitions
      %\item Safe timed transitions
      \item Safe Agent-Interactions
