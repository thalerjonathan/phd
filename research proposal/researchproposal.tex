...
\title{Research Proposal \\ An agent-based functional framework for the research of equilibrium-dynamics in bilateral trading and bartering.} % Title
...
\item Extend framework in the way that agents only learn from agents they have traded with instead from the whole agent-population as it seems unrealistic that a single agent is able to look at the whole agent-population and its performance - instead mutation should occur on local decisions.
...
\item Is the equilibrium-dynamic of the prices reproduce-able when using the private price mechanisms as claimed by Gintis?
\item Is the chaotic price-dynamic reproduce-able when using public price mechanisms as claimed by Gintis?
\item In which way does the model of Gintis change when agents learn only locally when trading with each other instead from the whole population?
