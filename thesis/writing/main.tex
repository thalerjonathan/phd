...
\begin{enumerate}
	\item finalise validating sugarscape appendix: wait for result runs

	\item Go through all chapters and make them first draft ready:
	\begin{itemize}
		\item Abstract (rephrase it)
		\item Introduction (rephrase it, more punch)
		\item Literature (more or less finished)
		\item Methodology: (needs overhaul and focussing, put SIR and Sugarscape in there, short monad transformer section in methodology)
		\item Implementing ABS: (more or less finished)
		\item Time-Driven ABS (not much work, nearly finished)
		\item Event-Driven ABS (probably more work)
		\item Generalising ABS (a lot of work, very drafty so far)
		\item SKIPPING PARALLEL ABS because it is first draft already
		\item SKIPPING PROPERTY-BASED because it is first draft already
		\item Discussion (a lot of work, only bits and pieces)
		\item Conclusion (a lot of work, only bits and pieces)
	\end{itemize}
\end{enumerate}
