...
\begin{enumerate}
	...
	\item MODERATE EFFORT: Discussion Drawbacks: finish efficiency	
	
	\item HIGH EFFORT: refine/rewrite event-driven implementation, it is too ad-hoc and the weakest (finished) chapter atm
...
\begin{enumerate}
	\item MANDATORY: write "Gintis Case Revisited" as it will be highly important for my internal assessor. Try to get the implementation working and refactor it into a functional implementation, atm it is awful

	\item OPTIONAL: implement total SD and ABS SIR implementation in Idris on a very simple level to make the point that we can (or cannot) make use of dependent types in this context to show the totality of a simulation under given equilibrium assumption.
\end{enumerate}
