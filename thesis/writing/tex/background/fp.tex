...
The function \texttt{randomShuffle} is overloaded, having the \texttt{MonadRandom} type class in its type constraints. This indicates that it is a monadic action where \texttt{m} is of type \texttt{MonadRandom}, which supports the same functionality as \texttt{Rand}. This is the major benefit \textit{mtl} provides, often resulting in much cleaner function types, which do not require fixing the order of the Monads in the stack. Another benefit is that we do not need lifts anymore. The drawback is that we cannot have multiple Monads of the same type, which would be still possible in a fully qualified Monad stack. The benefits become particularly clear when more than one effect is required. For example, we can write the type of \texttt{simulationCore} as:
...
\label{sec:back_frp}
Functional Reactive Programming (FRP) is a way to implement dataflow systems with continuous and discrete time semantics in pure functional languages. It has been successfully applied to implement GUIs, robotics, games and music. The original paper \cite{elliott1997functional} which invented this concept applied it in the context of graphical animation. It introduced so called \textit{behaviours} which are time-varying, reactive values, and \textit{events} which are sets of complex conditions. Behaviours are (reactive) values, which can change over time and react to events. Therefore, when interpreting images as behaviours they become animations. To make this more clear, we give an example of a behaviour \texttt{animatedPicture}, which returns a picture of a rotating solid circle with radius 100:

\begin{HaskellCode}
animatedPicture :: Time -> Picture
animatedPicture t = Translate (px, py) (circleSolid 100)
  where
    centerX = 30
    centerY = 30
    radius  = 100
    
    px = centerX + cos t * radius
    py = centerY + sin t * radius
\end{HaskellCode}

The animation is driven by accessing the changing \texttt{Time t}, transforming it into the circle coordinates. By now, there are many different approaches and implementations but, in this thesis, \textit{Arrowized} FRP \cite{hughes_generalising_2000, hughes_programming_2005} as implemented in the library Yampa \cite{courtney_yampa_2003,hudak_arrows_2003,nilsson_functional_2002} and Dunai \cite{perez_functional_2016} (see below) is used.
...
Arrowised FRP solves a fundamental flaw of early FRP, which is also visible in the example above: \texttt{Time} itself is exposed directly which could lead to non-causal signal functions. Consider the following example:
...
Using Arrowized FRP with its implementation of signal functions as arrows, it is possible to solve this issue, as arrows parametrise also over the input type, thus making it possible to deal with this issue. Technically speaking, an arrowized signal function is a continuation which allows to capture state using closures and hides away the $\Delta t$. This has the result, that the $\Delta t$ can be hidden and therefore never be exposed explicitly to the programmer. Consequently the programmer can neither manipulate it nor define non-causal systems where a signal function depends on a signal in the future. The way this is implemented in Yampa is through the \texttt{SF} type, which is a signal function defined at $t = 0$, which returns a continuation of type \texttt{SF'} which is the signal function defined for $t > 0$:
...
Both take an input \texttt{a} and return a value \texttt{b}. However at $t = 0$ no $\Delta t$ is available in the signal function, which is the one the programmer works with. At $t > 0$ all signal functions have changed into type \texttt{SF'} which is hidden by Yampas implementation and only visible to implementers of new signal functions. As an example we give the implementation of the \texttt{constant} signal function, which constantly returns value \texttt{b}, ignoring the input \texttt{a}:

\begin{HaskellCode}
constant :: b -> SF a b
constant b = SF {sfTF = tf0}
  where
    tf0 :: a -> (SF' a b, b)
    tf0 a = (sfConst b, b)
