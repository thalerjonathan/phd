...
Note the use of \textit{randomShuffle} and its type. It is an overloaded function, which has the \textit{MonadRandom} typeclass in its type-constraints. This indicates that it is a monadic action where \textit{m} is of type \textit{MonadRandom}, which supports the same functionality as \textit{Rand}. This is the major benefit \textit{mtl} provides, which often results in much cleaner function types and does not require to state fixed monad stacks in the type. The benefit is that we do not need lifts any more; the drawback is that we cannot have multiple Monads of the same type which would be still possible in a fully qualified Monad stack. The benefits becomes particularly clear when more than one effect is required, for example we could have written the type of \textit{simulationCore}
