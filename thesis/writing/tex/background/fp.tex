...
\begin{itemize}
	...
	\item Highly advance type system - Haskell has a strong static type system, which catches all type errors already at compile time and does not allow to bypass the type system (unless \texttt{coerce} or other cheating functions like \texttt{unsafePerformIO} are used). Further, Haskell is a \textit{pure} functional language and in our research it is absolutely paramount, that we focus on \textit{pure} functional ABS, which avoids any \texttt{IO} type under all circumstances. This property is enabled by the advanced type system and its strong static nature.
\end{itemize}
