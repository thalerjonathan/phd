...
\begin{itemize}
	\ite \textit{retry :: STM ()} - Retries an \textit{STM} block. This will cause to abort the current transaction and block the thread it is running in. When \textit{any} of the transactional data primitives has changed, the block will be run again. This is useful to await the arrival of data in a \textit{TVar} or put more concretely, to block on arbitrary conditions. 
	
	\iten \textit{orElse :: STM a -> STM a -> STM a}
\end{itemize}

TODO: describe retry and orElse

To run an \textit{STM} action the function \textit{atomically :: STM a $\to$ IO a} is provided, which performs a series of \textit{STM} actions atomically within an \textit{IO} context. It takes the STM action which returns a value of type \textit{a} and returns an \textit{IO} action which returns a value of type \textit{a}. This \textit{IO} action can only be executed within an \textit{IO} context, either within the main-thread or an explicitly forked thread.
