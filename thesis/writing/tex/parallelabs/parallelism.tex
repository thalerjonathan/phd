...
parallelism in haskell builds on laziness and two functions seq and par
...
Note: all timings are rough averages over multiple runs and not precise measurements because that is not the point here. we are only interested in showing what rough potential there is for speedup or if there is none - we are not interested in high performance computation here but rather in conceptual comparisons.
