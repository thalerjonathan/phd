...
The book \cite{marlow_parallel_2013} mentions that Par Monad and evaluation strategies result roughly in the same performance in most of the benchmarks. And indeed: we also applied Par Monad here to run the agents in parallel by evaluating their output and we get roughly the same speedup with no significant differences to the evaluation strategies: CPU bound is around factor 4 faster, IO bounds show basically no speedup at all and a combined approach where we print the output of the last simulation step before writing the aggregate to a file results in roughly the same speedup as the CPU bound only - so we can eat both cakes.