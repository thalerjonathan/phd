...
\begin{itemize}
	\item Underlying state - This is the state upon which the whole test operate. In our case the state is an agent and its continuations which encapsulate the current state the agent is in.
	
	\item An API to test against - This is the interface to manipulate the underlying state and present valid actions the test can carry out. Generally, in property-based testing in Haskell one generates actions to encode sequences of API calls. In our case the actions are obviously the events the agents receive as inputs and generate as outputs by scheduling them.
