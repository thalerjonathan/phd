...
[ ] find a killer example (model) for recursive ABS: must itself be recursive. search terms "recursive simulation", "recursive models".
[ ] what about each agent itself a full ABS? then its kind of a hierarchical ABS. but an ABS is a set if agents not a single agent? how can we abstract that? how can we see a many as a single? the messages and the environment for the containing (root) agent are simply distributed and forwarded to the subagents which are then simulated and reply with messages and changes in the environment. these subagents are simple replicas of the same agent but they must be hererogenous in some way otherwise they end up all with the same actions. maybe different time-sampling? maybe different rngs?
\end{abstract}
