...
After having encoded the invariants into a function, we need to write a QuickCheck property test which calls this function with random test data. Although QuickCheck comes with a lot of data generators for existing types like Strings, Integers, Double, List, it obviously does not have generators for custom types, like the \texttt{SIRState} and \texttt{SIREvent}. We refer to section \ref{sec:proptesting}, where we explain the concept of data generators and implement generators \texttt{SIRState} and \texttt{Probability}. The run-time generator \texttt{genEvent} for generating random \texttt{SIREvents} works similar to the \texttt{SIRState} generator and is omitted. For readers who are interested in a detailed implementation of \texttt{genEvent}, we refer to the code repository \cite{thaler_repository_2019}.
...
QuickCheck runs 100 test cases, prints the distribution of our labels and issues warnings in the last two lines that generated and expected coverages differ in these cases. Further, not all cases are covered, for example the contact with an \texttt{Infected} agent and the case of becoming infected. The reason for these issues is insufficient testing coverage as 100 test cases are simply not enough for a statistically robust result. We could increase the number of test cases to 100,000, which \textit{might} cover all cases but could still leave QuickCheck not satisfied as the expected and generated coverage \textit{might} still differ.
