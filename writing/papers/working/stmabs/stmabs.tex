...
%    sugarscape with array of MVars for access: we can run into deadlocks if 2 agents are aquitiring Locks because they are taken after another and therefore subject to races where they end up Holding a lock the other needs. the Problem here is that we cannot lock all of them in one go. locking, reading, unlocking then writing does not work bcs would overwrite: we need a consistent view of all cells.
...
% 4. add compositional example to background in STM: https://www.fpcomplete.com/blog/2016/11/comparative-concurrency-with-haskell
...
\begin{figure}[h!]
	...
	\caption{Performance comparison of \textit{Sequential}, \textit{STM}, \textit{Lock-Based} and \textit{Hybrid} SIR implementations under varying cores with grid size of 51x51 (2,601) agents.}
...
\begin{figure}[h!]
	...
	\caption{Performance comparison of \textit{Sequential}, \textit{Lock-Based}, \textit{TVar} and \textit{TArray} Sugarscape implementations under varying cores with 50x50 environment and 500 initial agents.}
