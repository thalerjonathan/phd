...
Next we give the implementation of the infectious stock (the implementations of the susceptible and recovered stock work similar and are left as a trivial exercise to the reader):
...
The stock receives flow from both the infection-rate and recovery-rate flow using the function \textit{flowInFrom} (see below). Then the current stock value is calculated using the \textit{integral} function of Yampa. Note that we can directly express the SD equation using Yampas DSL for continuous time-systems. The current stock value is then set as the observable value of the stock and sent to the infection-rate and recovery-rate flows. For convenience we implemented an additional function \textit{flowInFrom} which returns the value sent from the corresponding agent-id or 0.0 if none was sent.
...
The \textit{infection rate} flow is implemented as follows (the implementations of the recovery-rate flow works similar and is left as a trivial exercise to the reader):
...
Instead of integrating a value over time a stock just transforms incoming values from the connected stocks - in this case the susceptible and infectious stocks. Note again the direct expression of the formula for the infection rate.

When running the simulation one must make sure to use a small enough $\Delta t$ as \textit{integral} of Yampa is implemented using the rectangle rule which leads to considerable numerical errors with large $\Delta t$. Figure \ref{fig:sir_sd_dynamics} was created with our SD emulation in which we used a $\Delta t = 0.01$.
