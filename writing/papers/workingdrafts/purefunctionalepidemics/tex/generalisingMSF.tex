      ...
      if infected 
        then returnA -< (agentOut Infected, Event ())
          ...
          if isEvent makeContact
            then returnA -< 
              (dataFlow (contactId, Contact Susceptible) ao, NoEvent)
            else returnA -< (ao, NoEvent))
\end{HaskellCode}

Note also that \textit{drawRandomElemS} doesn't take a random number generator as well as it has been reimplemented to make full use of the MonadRandom in the stack:

\begin{HaskellCode}
drawRandomElemS :: MonadRandom m => SF m [a] a
drawRandomElemS = proc as -> do
  r <- getRandomRS ((0, 1) :: (Double, Double)) -< ()
  let a =  as !! floor (fromIntegral (length as) * r)
  returnA -< a
\end{HaskellCode}
