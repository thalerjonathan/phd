...
TODO: explain why analytical solution / why we need an ABS when we have an analytical solution anyway. The reason is that with the analytical solution we can validate our ABS implementation - the ABS approach is much more powerful in general as it allows to add spatiality as happening in section \ref{sec:step5_environment}, and have a heterogenous population - in SD its all averages.
...
TODO: we need a bit of clarification why we want to do ABS when we have an SD approach and what the differences are. SD allows for an analytical solution, but ABS can incorporate spatiality, heterogenity of population and can be applied when the global system behaviour is not known.
