...
The implementation of a susceptible agent in FRP is a bit more involved but much more expressive and elegant. We are using the Yampa primitive \textit{switch :: SF a (b, Event c) -> (c -> SF a b) -> SF a b} which runs a signal function until an event (the \textit{Event} type is equivalent to \textit{Maybe}) arises from it and then switches into the other signal function which is then run from that point in time.
...
TODO: put this before the code and explain dpSwitch
