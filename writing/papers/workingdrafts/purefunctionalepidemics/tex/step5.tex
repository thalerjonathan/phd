...
\begin{center}
	...
	\caption{Simulating the agent-based SIR model on a 21x21 2D grid with Moore neighbourhood and a single infected agent at the center and same SIR parameters. Simulation run until $t = 200$ with fixed $\Delta t = 0.1$. Last infected agent recovers around $t = 160$.}
	\label{fig:sir_env}
\end{center}
\end{figure}
...
\begin{enumerate}
	\item Passive read-only - implemented in previous steps, where the environment does never change and is passed as static information, e.g. list of neighbours, to each agent.
