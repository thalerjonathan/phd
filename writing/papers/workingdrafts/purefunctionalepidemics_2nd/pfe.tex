...
TODO: work external to the paper
	TODO fix RNG correlation bug and compare dynamics
	TODO look into lustre
	TODO look into simula 67

TODO: work internal to the paper
	TODO correlation in Yampa version: A) fix it or B) mention that this is actually a problem and simply wrong in Simulation (but not in a game), the next step solves that problem
	
	TODO SD SIR raus, nur kurz erwähnen, erklären dass ABS hier von vorteil ist
	
	TODO agent identity a bit lost: in FP the agent-identity is not as clear as in OOP where an agent can be hidden behind a polymorphic interface which is much more abstract in OOP than in FP. Also the identity of an agent is much clearer in OOP due to object-identity. Still we reach some level of identity through messages and explicit agent-interactions which we have omitted here due to lack of space and leave it for further research.
	...
	TODO notYet genauer erklären
	TODO add more details on under-sampling, can bring back in super-sampling idea.
	TODO simula 67 OOP and simulation have a history as oop emereged out of simula 67 which was created for simulation purposes, but in our research we look into different approaches
	...
	TODO dependent types: klarer machen was wir dort wollen (agent-interaction, internal state transitions, safe environment access, totality checking)
	
	TODO fix referee comment: In an English text citations should not be nouns (the authors of [x]), they are parenthesised additions. Removing them should leave a readable text. 
	TODO fix referee comment "You often say "The authors of [XY] discuss/present ...". This is bad for two reasons. First of all, numeric references should
