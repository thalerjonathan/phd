...
In this step we followed the FRP approach which is fundamentally different than from the previous step in section \ref{sec:naive_beginnigs} because in FRP we assume a continuous flow of time as opposed to discrete time-steps of $\Delta t = 1.0$ previously. This means that we need to select a \textit{correct} $\Delta t$ otherwise we would end up with wrong dynamics. The selection of a correct $\Delta t$ depends in our case on \textit{occasionally}, which randomly generates an event on average with a given rate following the exponential distribution. To arrive at the correct dynamics, this requires us to sample \textit{occasionally}, and thus the whole system, with small enough $\Delta t$ which matches the rate. If we choose a too large $\Delta t$, we loose events which will result in wrong dynamics as can be seen in Figure \ref{fig:sir_abs_approximating_01dt_1000agents}. This issue is known as under-sampling and is described in Figure \ref{fig:sampling_issue}.
...
\begin{center}
	...
	\caption{A visual explanation of under-sampling and super-sampling. The black dots represent the time-steps of the simulation, which can be understood as samples of the continuous system at the respective times. The red dots represent events which occur at a specific point in time. In the case of under-sampling, 3 events occur in between the two time steps but \textit{occasionally} only captures the first one. In super-sampling the time-step is divided into 3 equally spaced intervals which results in all 3 events being captured.} 
	\label{fig:sampling_issue}
\end{center}
\end{figure}

Clearly increasing the $\Delta t$ results in a closer approximation to the reference dynamics as can be seen \ref{fig:sir_abs_approximating_001dt_1000agents}. To increase the accuracy we have two options. Either we use even smaller $\Delta t$ which results in the whole system being sampled more often, thus reducing performance, or we implement super-sampling and apply it to \textit{occasionally} which would allow us to run the whole simulation with $\Delta t = 1.0$ and only sample the \textit{occasionally} function with a much higher frequency. An approach would be to introduce a new combinator to Yampa which allows to super-sample other signal functions. 
