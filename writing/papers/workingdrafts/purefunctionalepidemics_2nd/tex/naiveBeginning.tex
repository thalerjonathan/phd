...
\begin{enumerate}
	\item We are using a hard-coded $\Delta t = 1.0$ because of the way our susceptible behaviour is implemented which is not very elegant and general.
	\item $\Delta t$ is passed explicitly as argument to the agent and needs to be dealt with explicitly in infected behaviour. This is not very elegant and a potential source of errors - can we do better and find a more elegant solution? 
