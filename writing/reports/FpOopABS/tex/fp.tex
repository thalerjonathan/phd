...
\section{Theoretical Foundation: Lambda Calculus}
...
%TODO: short explanation of how the lambda calculus works: alpha-equivalence/conversion, beta-reduction, eta-conversion, free variables. Explanation mostly inspired from \cite{maclennan_functional_1990} and \cite{allen_haskell_2016} with a bit of \url{https://en.wikipedia.org/wiki/Lambda_calculus}
...
$(\lambda x.x)(\lambda x.x)$ applies the identity function to itself (note that functions can be passed as arguments to functions - they are \textit{first class} in the Lambda Calculus) - this results in the same expression again and is thus a non-terminating expression.

\medskip

We can formulate simple arithmetic operations like addition of natural numbers using the Lambda Calculus. For this we need to find a way how to express natural numbers. This problem was already solved by Alonzo Church by introducing the Church numerals: a natural number is a function of an n-fold composition of an arbitrary function f. The number 0 would be encoded as $0 = \lambda f . \lambda x.x$, 1 would be encoded as $1 = \lambda f . \lambda x . f x$ and so on.
...
\section{Theoretical Foundation: Types}
TODO: look into Haskell's Type-System
