...
\begin{enumerate}
	...
	\item Computational or judgmental equality: two terms "2 + 2" and "4" are said to be computationally equal because when the result of the addition is computed by a program then they will reduce to the same term S(S(Z)) + S(S(Z)) = S(S(S(S(Z)))). Very often this kind of equality is treated under definitional equality.
	\item Propositional equality: when one wants to define general rules that e.g. "a+b" and "b+a" are equal, we are talking about a theorem, not a definition. Computational / definitional equality does not work here as to compute it one needs to substitute a and b for concrete natural numbers. In this case we are talking about extensional equality, which is a judgement, not a proposition and thus \textit{not} internal to the formal system itself. It can be internalized through \textit{propositional} equality
\end{enumerate}

Now it becomes more clear what a \textit{intensional} type theory means: TODO. An \textit{extensional} type theory on the other hand: TODO.

Applied to our examples this means the following: We have definitional/intensional equality AND 
Propositional equality is exactly what we got when we introduced the identity type above in the checkEqNat function with Dec (num1 = num2). (the Dec type indicates that it is decidable and may or may not be inhabited)
