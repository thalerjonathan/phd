...
\begin{enumerate}
	...
	\item stm paper: we can do parallel and concurrent ABS in haskell without the low level pain. journal: https://tomacs.acm.org/
	\item sd paper: we can do system dynamics in functional programming as well arriving at a correct-by-construction approach. conference e.g. experience paper to ICFP?
	\item sync communication in FRP: how can signal functions communicate with each other synchronously? this means basically that we change the network structure dynamically all the time. in the end its a pure functional way of method calls. journal
	\item dependent types in ABS: present an indexed monad for ABS which allows correct by construction ABS following a shallow embedded EDSL. conference.
