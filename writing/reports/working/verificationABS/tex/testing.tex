...
Agents \textit{might} behave as signals in FRP which means that if they are a signal they should stay constant if time stays constant. This means that they should not change in case one is sampling the system with $\Delta t = 0$. Of course to test whether this will \textit{always} be the case is strictly speaking impossible with a Black Box verification but we can guarantee much more when staying pure and by white box verification we can really guarantee this property.
